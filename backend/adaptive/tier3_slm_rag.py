...
from backend.adaptive.slm_interface import get_inference_provider, InferenceResult
...
class Tier3Adaptive:
    ...
    async def adapt(
        ...
        try:
            ...
            # If inference fails, try to build from structural hints + RAG
            detail = {
                "retrieved_templates": [],
                "inference_mode": inference_mode,
                "inference_detail": f"Inference failed: {e}",
                "confidence": 0.0,
                "tier": "TIER-3 ADAPTIVE",
                "error": str(e),
            }
